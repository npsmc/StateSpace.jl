...
"""
struct FilteredState{T, D<:ContinuousMultivariateDistribution}
...
function show(io::IO, fs::FilteredState{T}) where T
    n = length(fs.state)
...
import Statistics: mean, var, cov, cor, rand

for op in (:mean, :var, :cov, :cor, :rand)
    @eval begin
        function ($op)(s::FilteredState{T}) where T
